...
std::pair<std::size_t, std::size_t> two_sum(const std::vector<int> &Nums,
                                            int Target) {
  std::map<std::size_t, std::size_t> M;
  for (std::size_t i = 0; i < Nums.size(); i++) {
    auto Complemenet = Target - Nums[i];
    if (M.find(Complemenet) != M.end())
      return {M[Complemenet], i};
    M.insert(std::pair<std::size_t, std::size_t>(Nums[i], i));
  }
  return {0, 0};
}
