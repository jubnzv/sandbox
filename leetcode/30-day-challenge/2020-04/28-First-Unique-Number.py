...
class FirstUnique:
    ...
    def __init__(self, nums: List[int]):
        ...
        self.counter_unique = {k: v for k,
                               v in Counter(nums).items() if v == 1}

    def showFirstUnique(self) -> int:
        return next(iter(self.counter_unique)) if self.counter_unique else -1

    def add(self, value: int) -> None:
        if value not in self.counter:
            self.counter[value] = 1
            self.counter_unique[value] = 1
        else:
            self.counter[value] += 1
            if value in self.counter_unique:
                del self.counter_unique[value]
